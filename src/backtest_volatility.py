...
def create_deciles(df):
    """
    Create monthly deciles based on volatility.
    Lower volatility = better, so decile_0 will be lowest volatility.
    """
    df = df.copy()
    df = df.dropna(subset=["volatility", "ret_fwd_1m"])
...
def compute_long_short(pivot):
    ...
    Since decile_0 = lowest volatility and decile_9 = highest volatility,
    long_short = decile_0 - decile_9
    """
    pivot = pivot.copy()
    ...
    low_vol_col = decile_cols[0]
    high_vol_col = decile_cols[-1]

    pivot["long_short"] = pivot[low_vol_col] - pivot[high_vol_col]
...
def compute_cumulative_returns(pivot):
    """
    Convert monthly returns into cumulative return series.
    """
...
def summarize_performance(pivot):
    """
    Print and return summary performance metrics.
...
def main():
    ...
    input_file = data_dir / "volatility_factor.csv"
    output_file = data_dir / "volatility_backtest.csv"
    summary_file = data_dir / "volatility_backtest_summary.csv"
    cumulative_file = data_dir / "volatility_cumulative_returns.csv"
